...
Num_Bank_Accounts = st.selectbox('Enter Number of bank accounts', data['Num_Bank_Accounts'].unique().tolist())
Num_Credit_Card = st.selectbox("Enter number of credit cards", data['Num_Credit_Card'].unique().tolist())
Interest_Rate = st.number_input("Enter interest rate", step=0.01, format="%.2f")
Num_of_Loan = st.selectbox("Enter number of loans", data['Num_of_Loan'].unique().tolist())
